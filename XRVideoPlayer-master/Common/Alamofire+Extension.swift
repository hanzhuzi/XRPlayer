...
extension Request {
    ...
    // 解析
    public class func responseJSONSerializer() -> ResponseSerializer<AnyObject, NSError> {
        
        return ResponseSerializer { _, _, data, error in
            
            guard error == nil else { return .Failure(error!) }
            
            guard let ralData = data where ralData.length > 0 else {
    ...
    public func responseJSONSerializer(queue: dispatch_queue_t?, keyPath: String? = nil, complationHandle: (NSURLRequest?, NSHTTPURLResponse?, AnyObject?, NSError?) -> Void) -> Self {
